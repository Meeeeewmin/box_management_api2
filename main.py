...
DATABASE_URL = os.getenv("DATABASE_URL", "mysql+pymysql://iot_user:iot_password@localhost:3306/iot_box_db")
...
class BoxBase(BaseModel):
    ...
    def validate_mac(cls, v):
        ...
        if not re.match(pattern, v):
            raise ValueError('유효하지 않은 MAC 주소 형식입니다')
        return v.upper()
...
def get_boxes(
    ...
    search: Optional[str] = Query(None, description="검색어 (MAC, IP, 설비, 위치, 공정)"),
    process: Optional[str] = Query(None, description="공정 필터"),
    db: Session = Depends(get_db)
):
    """IoT 박스 목록 조회 (페이지네이션 + 검색)"""
